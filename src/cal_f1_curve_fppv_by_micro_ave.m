function detcurve = cal_f1_curve_fppv_by_micro_ave(Res, param)
...
%     neg_sorted_score = sorted_score_array(idneg);
    ...
    TP_array = zeros(1,length(num_fp_array)) ;
    FP_array = zeros(1,length(num_fp_array)) ;
    FN_array = zeros(1,length(num_fp_array)) ;
...
%         end
    ...
    TP_crosses = [TP_crosses ; TP_array];
    FP_crosses = [FP_crosses ; FP_array];
...
FN_crosses = sum(FN_crosses);


Precision_Arr = TP_crosses ./ (TP_crosses + FP_crosses);
Recall_Arr = TP_crosses ./ (TP_crosses + FN_crosses);
F1_Array = (2 * Precision_Arr.* Recall_Arr) ./ (Precision_Arr + Recall_Arr);
detcurve.xaxis = Recall_Arr;
% there are -1 in some cross: remove calculate -1 in each 
detcurve.yaxis = F1_Array;
detcurve.ystd = [];
