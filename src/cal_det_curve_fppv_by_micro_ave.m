function detcurve = cal_det_curve_fppv_by_micro_ave(Res, param)
...
MissRate_Arr = [];
FN_array_of_crosses = [];
num_ME_seq = zeros(1,size(num_fp_array,2));
FP_video = {};
fpvid = 1;
...
%     neg_sorted_score = sorted_score_array(idneg);
    ...
    num_ME_seq = num_ME_seq + num_POS_ME;
...
%         end
    ...
    FN_array_of_crosses = [FN_array_of_crosses ; FN_array];
end

toc
detcurve.xaxis = num_fp_array / num_test_video;
% there are -1 in some cross: remove calculate -1 in each 
detcurve.yaxis = sum(FN_array_of_crosses) ./ num_ME_seq;
detcurve.ystd = [];
